...
//批量产品的时候，很容易出现外部晶振不起振的情况，不太可靠
    ...
    {
		#if	PRINTF_EN
        printf("extern RTC running!\r\n");
		#endif
    }
    else
    {
		#if	PRINTF_EN
        printf("start extern RTC failure!\r\n");
		#endif
    }
}
 ...
 */
//时间戳 -> 北京时间并打印
void Time_Printf(uint32_t TimeVar,struct rtc_time *tm)
{
    uint32_t BJ_TimeVar;
    ...
    /* 输出时间戳，公历时间 */
    App_printf("UNIX Stamp = %d	%d-%d-%d	%0.2d:%0.2d:%0.2d\r\n",
               TimeVar,
...
//时间戳 -> 北京时间
void update_RTCtime(uint32_t TimeVar,struct rtc_time *tm)
{
    uint32_t BJ_TimeVar;
    ...
    //更新时间，不打印
//    App_printf("update_RTCtime...\r\n");
//    App_printf("UNIX Stamp = %d	%d-%d-%d	%0.2d:%0.2d:%0.2d\r\n",
//				TimeVar,
//				tm->tm_year, tm->tm_mon, tm->tm_mday,
//				tm->tm_hour, tm->tm_min, tm->tm_sec);

//    App_printf("UNIX时间戳 = %d 当前时间为: %d年(%s年) %d月 %d日   %0.2d:%0.2d:%0.2d\r\n",TimeVar,
