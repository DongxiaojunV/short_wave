...
#define TRUE_W5500	0xff
#define FALSE_W5500	0x00
...
#define W5500_INT		GPIO_Pin_11	//定义W5500的INT引脚
...
extern unsigned char S0_Port[2];	//端口0的端口号(1225) 
extern unsigned char S0_DIP[4];		//端口0目的IP地址 
extern unsigned char S0_DPort[2];	//端口0目的端口号(6000) 

extern unsigned char S1_Port[2];	//端口1的端口号
extern unsigned char S1_DIP[4];		//端口1目的IP地址 
extern unsigned char S1_DPort[2];	//端口1目的端口号

extern unsigned char S2_Port[2];	//端口2的端口号
extern unsigned char S2_DIP[4];		//端口2目的IP地址 
extern unsigned char S2_DPort[2];	//端口2目的端口号

extern unsigned char S3_Port[2];	//端口2的端口号
extern unsigned char S3_DIP[4];		//端口2目的IP地址 
extern unsigned char S3_DPort[2];	//端口2目的端口号

extern unsigned char UDP_DIPR[4];	//UDP(广播)模式,目的主机IP地址
extern unsigned char UDP_DPORT[2];	//UDP(广播)模式,目的主机端口号

extern unsigned char NTP_DIPR[4];	//UDP(广播)模式,NTP服务器主机IP地址
extern unsigned char NTP_DPORT[2];	//UDP(广播)模式,NTP服务器主机端口号
...
extern unsigned char S3_Mode;	//端口2的运行模式,0:TCP服务器模式,1:TCP客户端模式,2:UDP(广播)模式
...
extern unsigned char S3_State;	//端口2状态记录,1:端口完成初始化,2端口完成连接(可以正常传输数据) 
...
/***************----- 端口数据缓冲区 -----***************/
extern unsigned char Socket0_Rx_Buffer[512];	//端口0接收数据缓冲区 
extern unsigned char Socket0_Tx_Buffer[256];	//端口0发送数据缓冲区 
extern unsigned char Socket1_Rx_Buffer[2048];	//端口1接收数据缓冲区 
extern unsigned char Socket1_Tx_Buffer[128];	//端口1发送数据缓冲区 
extern unsigned char Socket2_Rx_Buffer[128];	//端口2接收数据缓冲区 
extern unsigned char Socket3_Rx_Buffer[128];	//端口3接收数据缓冲区 
...
extern void SPI_Configuration(void);//W5500 SPI初始化配置(STM32 SPI2)
...
extern unsigned short Read_SOCK_Data_Buffer(SOCKET s, uint8_t *dat_ptr);//指定Socket(0~7)接收数据处理
extern void Write_SOCK_Data_Buffer(SOCKET s, uint8_t *dat_ptr, uint8_t size); //指定Socket(0~7)发送数据处理
extern void Socketn_Char_Send(SOCKET s,char *dat_ptr, uint8_t size);			//指定Socket(0~7)发送char类型数据
extern void W5500_Process(void);//W5500处理程序框架
extern void W5500_Initialization(void);
extern void Load_Net_Parameters(void);
extern void W5500_Socket_Set(void);
extern void Get_STM32_ID(void);
extern void NVIC_Configuration(void);
extern void Timer3_Init_Config(void);
extern void TIM3_IRQHandler(void);
extern void System_Initialization(void);
extern void RCC_Configuration(void);
extern void Process_Socket_Data(SOCKET s,uint8_t * msg, uint8_t len);
extern void My_Delay_xms(uint32_t ms);
void My_Delay_ms(uint32_t ms);
extern unsigned char Read_W5500_1Byte(unsigned short reg);
extern void Write_W5500_SOCK_4Byte(SOCKET s, unsigned short reg, unsigned char *dat_ptr);//向目的地址寄存器写入与本机IP不同的IP值
extern unsigned char Read_W5500_SOCK_1Byte(SOCKET s, unsigned short reg);
extern uint16_t W5500_rx_buf_len;
extern uint16_t RxBuf_len;
#endif
